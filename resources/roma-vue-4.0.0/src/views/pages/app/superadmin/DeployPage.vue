            ...
            <div class="grid">
                <div class="col-6">
                    <div class="card flex justify-content-left ">
                        <div class="flex flex-column gap-3">
                            <div v-for="(value,key) in topbarGenericConfig" :key="key" class="flex w-full gap-1">

                                <label :for="key">{{ key}}</label>
                                <InputText class="flex-grow-1"  v-model="topbarGenericConfig[key]" :name="key" :value="value" />
                            </div>
                        </div>
                    </div>
                </div>
                <div class="col-6">
                    <div class="card flex justify-content-center">
                        ...
                        <div class="flex flex-column gap-3">
                            <div v-for="category of topbarBooleanConfig" :key="category.label" class="flex align-items-left gap-2 w-full">
                                <Checkbox :multiple="true" @change="changeTopBarConfig" v-model="selectedCategories"  :inputId="category.label" name="category" :value="category.label" />
...
<!--            <div v-for="category of topbarConfig" :key="category.value" class="flex align-items-center">-->
<!--                <Checkbox @change="changeTopBarConfig" v-model="selectedCategories" :inputId="category.label" name="category" :value="category.value" />-->
...
<!--            </div>-->
            <div class="flex">
                ...
                <div>
                    ...
                    <div class="fa fa-circle text-2xl m-2 green blob" :class="serverStatus?'text-green-500 ':'text-red-500 '"></div>
                </div>
            </div>
            ...
            <div class="flex gap-2">
                ...
                <Button label="Compilazione Templates" @click="generateTemplates"></Button>
...
export default {
    ...
    data() {
        let dt =  Object.assign(templateConfig,{
            ...
            bufferOut : '',
            bufferErr : '',
            topbarBooleanConfig : Utility.jsonToProperties(topbarJson.boolean),
            topbarGenericConfig : Object.assign({},topbarJson.generic),
            selectedCategories : [],
        });
        console.debug('dt',dt);
        for (let k in topbarJson.boolean) {
            if (topbarJson.boolean[k]) {
                dt.selectedCategories.push(k);
            }
        }
        window.CAT = dt.selectedCategories;
        return dt;
    },
    methods : {
        changeTopBarConfig() {
            console.debug('changeTopBarConfig',this.selectedCategories);
        ...
        saveTopbarConfig() {
            let confs = Object.assign({},topbarJson);
            console.debug('confs',confs);
            for (let k in confs.boolean) {
                console.log('k',k);
                if (this.selectedCategories.indexOf(k) >=0) {
                    confs.boolean[k] = true;
                } else {
                    confs.boolean[k] = false;
                }
            }
            console.debug('nuove configurazioni',confs)
            let json = {
                action : 'save-topbarconfig',
                filename : 'topbar.json',
                body : confs
            }
            connection.send(JSON.stringify(json));
            // setTimeout(function () {
